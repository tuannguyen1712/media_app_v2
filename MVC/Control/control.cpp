...
void Application::print_files_in_pages(std::vector<std::string> files, const int &page, const int &size, const int &total_page)
{
    ...
    for (int i = page * FILES_PER_PAGE + 1; i <= (page + 1) * FILES_PER_PAGE && i <= size; i++)
    {
        if (pathExists(const_cast<char *>(files[i - 1].c_str())))
        {
            ...
            Screen_stack.top()->set_metedata(Media("File not exist", "N/A", "N/A", 0, 0));
        }
...
void Application::thread_play_media() 
{
    while (is_play) {
        ...
        if (list_files.empty()) {
            total_page = (size % FILES_PER_PAGE == 0) ? (size / FILES_PER_PAGE) : (size / FILES_PER_PAGE) + 1;
            print_files_in_pages(List_playlist[pli].__list, page, size, total_page);
            Screen_stack.top()->set_metedata(GetMedia(List_playlist[pli].__list[media_file_index]));
        }
        else {
            total_page = (size % FILES_PER_PAGE == 0) ? (size / FILES_PER_PAGE) : (size / FILES_PER_PAGE) + 1;
